...
with(dat, table(dat$CollectYear.1, PriMethod)) # number of captures by year and method
...
sink("sicklefin_redhorse_ms_js.jags")
cat("
    model{
    phi ~ dunif(0,1) # Annual survival prob
    #alpha0 ~ dnorm(0,0.37)
    ...
    cap.probs[1,7] <- 0
    cap.probs[1,8] <- 0
    cap.probs[2,1] <- (1-pSeine)*(1-pFyke)*(1-pPit) # Pr(not detected|alive)
    cap.probs[2,2] <- pSeine*(1-pFyke)*(1-pPit) # Pr(detected in seine, but not by other gear)
    cap.probs[2,3] <- pFyke*(1-pSeine)*(1-pPit)
    cap.probs[2,4] <- pPit*(1-pFyke)*(1-pSeine)
    cap.probs[2,5] <- pFyke*pPit*(1-pSeine)
    cap.probs[2,6] <- pSeine*pFyke*(1-pPit)
    cap.probs[2,7] <- pSeine*pPit*(1-pFyke)
    cap.probs[2,8] <- pSeine*pFyke*pPit


    cap.probs[3,1] <- 1 # If dead, can't be detected
    cap.probs[3,2] <- 0
    cap.probs[3,3] <- 0
    cap.probs[3,4] <- 0
    cap.probs[3,5] <- 0
    ...
    for (t in 2:n.occ){
    z[i,t] ~ dcat(ps[z[i,t-1], ]) # Actual alive/dead state (partially observed)
    ## state[i,t] <- z[i,t]+1
...
# turn observation "states" to latent states for initial value
zi <- ms.js.CH.aug
for(i in 1:nrow(zi)) {
  for(j in 1:ncol(zi)){
...
# Parameters monitored
params <- c("pFyke", "pSeine", "pPit", "phi", "gamma", "Nsuper", "N", "B")

# Data
in.data <- list(y = ms.js.CH.aug, M = dim(ms.js.CH.aug)[1], n.occ = dim(ms.js.CH.aug)[2])
...
sr.ms.js.jm1<-jags.model(data=in.data, inits = inits, file = "sicklefin_redhorse_ms_js.jags",
                         n.chains = nc, n.adapt = nb, quiet = F)
